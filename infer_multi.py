...
@click.option("--output_dir", type=str, required=True, default=None, help="Directory to save the outputs")
@click.option("--full_audio_dir", type=str, required=True, default=None, help="Path to the full audio directory, to get the list of audio prompts to load")
@click.option("--audio_prompt_dir", type=str, required=True, default=None, help="Path to the start and end audio prompt directory")
@click.option("--concat_audio_dir", type=str, required=True, default=None, help="Path to the middle segment noised audio directory to save and load from")
...
@click.option("--gen_duration", type=float, default=10.0, help="The duration of the transition to be generated, same for every audio prompt for now")
def main(checkpoint_path, bf16, torch_compile, cpu_offload, overlapped_decode, device_id, output_dir, full_audio_dir, audio_prompt_dir, concat_audio_dir, repaint_variance, gen_duration):
    ...
    #Run the preprocessing on cpu since the concat process should be quite low cost 
    device = torch.device("cpu")
    #loop through each prompt to generate
    full_track_paths = [file for file in Path(full_audio_dir).glob("*.mp3") if file.is_file()]

    for full_track_path in full_track_paths:
        track_name = full_track_path.stem
        start_prompt_path = os.path.join(audio_prompt_dir, track_name + '.beginning.mp3')
        end_prompt_path = os.path.join(audio_prompt_dir, track_name + '.end.mp3')
        if not os.path.exists(start_prompt_path):
            raise FileNotFoundError(f"[ERROR] Starting prompt for audio {str(full_track_path)} does not exist!")
        if not os.path.exists(end_prompt_path):
            raise FileNotFoundError(f"[ERROR] Ending prompt for audio {str(full_track_path)} does not exist!")
        concat_audio_path = os.path.join(concat_audio_dir, track_name + '.noised.mp3')
        
        repaint_start, repaint_end = prompts_concat(start_prompt_path, end_prompt_path, concat_audio_path, gen_duration, device)
        src_audio_path=concat_audio_path
        output_path = os.path.join(output_dir, track_name + '.gen.mp3')
