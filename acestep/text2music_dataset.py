...
class Text2MusicDataset(Dataset):
    ...
    def get_noised_audio(self, item):
        """
        Load and preprocess the middle segment noised audio file
        ...
        Returns:
            torch.Tensor or None: Processed audio tensor
        """
        filename = item["noised_filename"]
        sr = 48000
        try:
            audio, sr = torchaudio.load(filename)
        except Exception as e:
            logger.error(f"Failed to load noised audio {item}: {e}")
            return None

        if audio is None:
            logger.error(f"Failed to load noised audio {item}")
            return None
    ...
    def process(self, item):
        ...
        noised_music_wavs = noised_audio
        ...
        #make sure the noised audio is the same length
        noised_music_wavs = noised_music_wavs[:, :wav_len]
