...
class Pipeline(LightningModule):
    ...
    def preprocess(self, batch, train=True):
        ...
        noised_wavs = batch["noised_wavs"]
        wav_lengths = batch["wav_lengths"]

        #assume target_wavs are all of same type as noised_wavs
        ...
        noised_latents, _ = self.dcae.encode(noised_wavs, wav_lengths)
    ...
    def run_step(self, batch, batch_idx):
        ...
        # Input is the latent from the noised audio
        # IMPORTANT: 'noised' here means that the audio signal that the latent originated from was noised, not the latent
        target_image = target_latents
        noised_image = noised_latents

        device = noised_image.device
        dtype = noised_image.dtype
        # Step 1: Generate random noise, initialize settings
        noise = torch.randn_like(noised_image, device=device)
        bsz = noised_image.shape[0]
        timesteps = self.get_timestep(bsz, device)

        # Add noise according to flow matching. 
        sigmas = self.get_sd3_sigmas(
            timesteps=timesteps, device=device, n_dim=noised_image.ndim, dtype=dtype
        )
        noisy_image = sigmas * noise + (1.0 - sigmas) * noised_image

        # This is the flow-matching target for vanilla SD3.
        # Target is the latent from the clean audio
        target = target_image
